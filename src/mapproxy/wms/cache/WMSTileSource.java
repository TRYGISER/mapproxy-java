...
public class WMSTileSource extends TileSource {

    private int meta_buffer = 0;
    private int[] meta_size = new int[]{2, 2};
    private TileGrid grid;
    private WMSClient[] clients;
   ...
   */
    public WMSTileSource(TileGrid grid, WMSClient[] clients, String format,
    ...
    public WMSTileSource(TileGrid grid, WMSClient[] clients){
        this(grid, clients, null, 0, new int[]{2, 2});
    }

    public WMSTileSource(TileGrid grid, WMSClient client){
        this(grid, new WMSClient[]{client});
    }
  ...
  //**************************************************************************
    ...
    private boolean _has_transparent_sources(){
        ...
        for (WMSClient client : this.clients){
  ...
  //**************************************************************************
    ...
    private javaxt.io.Image _get_meta_tile(_Tile tile){
        ...
        //System.out.println("tile.coord: " + mapproxy.core.Python.cstr(tile.coord));
        //System.out.println("bbox: " + mapproxy.core.Python.cstr(bbox));
        //System.out.println("size: " + mapproxy.core.Python.cstr(size));
        java.util.List<javaxt.io.Image> responses = new java.util.ArrayList<javaxt.io.Image>();
        for (WMSClient client : this.clients){ //for client in self.clients:
            WMSMapRequest request = new WMSMapRequest();
            request.params.bbox(bbox);
            request.params.size(size);
            request.params.set("srs", grid.srs.toString()); //<-- Had to add this. Not sure why this isn't in the original python code...
            javaxt.io.Image img = client.get_map(request);
            responses.add(img);
        }
